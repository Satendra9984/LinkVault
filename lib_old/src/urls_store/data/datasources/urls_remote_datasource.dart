...
abstract class UrlsRemoteDataSource {
  ...
  Future<List<UrlModel>> moveUrls(
      List<String> urlIds, String targetCollectionId);
...
class UrlsRemoteDataSourceImpl implements UrlsRemoteDataSource {
  ...
  Future<UrlModel> createUrl(UrlModel url) async {
    try {
      ...
      return UrlModel.fromJson(response);
  ...
  Future<List<UrlModel>> getUrlsByCollection(
    ...
    try {
      ...
      return response.map<UrlModel>((json) => UrlModel.fromJson(json)).toList();
    } on PostgrestException catch (e) {
      throw UrlRemoteException(
        message: e.message,
        statusCode: e.code ?? 'UNKNOWN',
  ...
  Future<List<UrlModel>> createUrls(List<UrlModel> urls) async {
    try {
      final urlMaps = urls.map((url) => url.toJson()).toList();

      final response =
          await _supabaseClient.from(_tableName).insert(urlMaps).select();
  ...
  @override
  Future<List<UrlModel>> moveUrls(
      List<String> urlIds, String targetCollectionId) async {
    try {
      final response = await _supabaseClient
          .from(_tableName)
  ...
  Future<void> reorderUrls(String collectionId, List<String> orderedIds) async {
    try {
      ...
      await _supabaseClient.from(_tableName).upsert(updates, onConflict: 'id');
    } on PostgrestException catch (e) {
      throw UrlRemoteException(
        message: e.message,
        statusCode: e.code ?? 'UNKNOWN',
  ...
  @override
  Future<UrlModel> updateMetadata(
